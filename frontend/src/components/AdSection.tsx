...
const AdSection: React.FC = () => {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-4 sm:py-8">
      <div className="group relative min-h-[320px] sm:h-96 w-full overflow-hidden rounded-[1.5rem] sm:rounded-[2rem] shadow-2xl ring-1 ring-white/10 transition-all hover:ring-primary/50">
        <img 
          src={currentAd.imageUrl} 
          alt={currentAd.title} 
          className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-110"
        />
        <div className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/40 to-transparent flex flex-col justify-center px-6 sm:px-12 md:px-20 py-8 sm:py-10">
          {currentAd.badgeText && (
          ...
          <h2 className="text-2xl sm:text-4xl md:text-6xl font-black text-white mb-2 sm:mb-3 tracking-tighter drop-shadow-2xl uppercase leading-tight">
            {currentAd.title}
          </h2>
          
          {currentAd.description && (
            <p className="max-w-md text-xs sm:text-base md:text-xl text-gray-200 mb-6 sm:mb-8 font-medium leading-relaxed drop-shadow-lg">
              {currentAd.description}
            </p>
          )}
